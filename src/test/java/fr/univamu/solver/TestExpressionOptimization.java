...
public class TestExpressionOptimization {
    ...
    private Variable createOptimizedExpression(Solver solver, Object... terms) {
        try {
            var method = Solver.class.getDeclaredMethod("expression", boolean.class, Object[].class);
            method.setAccessible(true);
            return (Variable) method.invoke(solver, true, terms);
        } catch (Exception e) {
            fail("Impossible de créer une expression optimisée: " + e.getMessage());
            return null;
        }
    }
